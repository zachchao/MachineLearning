...
if __name__ == '__main__':
    ...
    train_x, train_y = getAllTA(symbols[0]["symbol"], (days), time() - ((days + 1) * 68400))

    for i in range(1, 311):
        if hasExistedForAtLeast(symbols[i]["symbol"], days + 1):
            temp_train_x, temp_train_y = getAllTA(symbols[i]["symbol"], (days), time() - ((days + 1) * 68400))

            train_x = pd.concat([train_x, temp_train_x])
            train_y += temp_train_y


    # Today
    test_x, test_y = getAllTA(symbols[0]["symbol"], 1, time() - (1 * 10000))

    for i in range(1, 311):
        if hasExistedForAtLeast(symbols[i]["symbol"], days + 1):
            temp_test_x, temp_test_y = getAllTA(symbols[i]["symbol"], 1, time() - (1 * 10000))

            test_x = pd.concat([test_x, temp_test_x])
            test_y += temp_test_y


    print(test_x.columns.values)

    with open("sentiment_set2.pickle", 'wb') as f:
